...
function validate(ast, filePath, source) {
  ...
  walk.full(ast, (node) => {
     ...
     * We only reject at validation time when we can prove at the AST level
     * that the expression is DEFINITIVELY wrong — i.e. both operands are
     * known string types (e.g. two string literals, or a template literal
     * with a member expression).
     *
     * If either operand is an Identifier, we defer to the transform pass
     * which has a symbol table and will:
     *   - emit C arithmetic if both resolve to numeric
     *   - rewrite to IRConcat (snprintf) if either resolves to string
     *   - throw a contextual compile-time error if the type is unresolvable
     */
    if (node.type === "BinaryExpression" && node.operator === "+") {
      const leftIsString  = isStringNode(node.left);
      const rightIsString = isStringNode(node.right);
      const leftIsIdent   = node.left.type  === "Identifier";
      const rightIsIdent  = node.right.type === "Identifier";

      /* If either side is an identifier, defer to transform */
      ...
      /* identifier on either side → deferred to transform pass */
    }
  });
