...
/**
 * Returns true if an AST node is definitively a string-typed value.
 * These map to `const char *` in C and cannot participate in `+` arithmetic.
 */
function isStringNode(node) {
  if (!node) return false;
  /* String literal */
  if (node.type === "Literal" && typeof node.value === "string") return true;
  /* Template literal — always a string */
  if (node.type === "TemplateLiteral") return true;
  /* Member expressions that resolve to string runtime values */
  if (node.type === "MemberExpression") return true;
  return false;
}

/**
 * Returns true if an AST node is definitively numeric (safe for C arithmetic).
 * Only numeric literals and binary/unary arithmetic on numerics qualify.
 */
function isNumericNode(node) {
  ...
  if (
    ...
    return isNumericNode(node.left) && isNumericNode(node.right);
  }
  if (node.type === "UnaryExpression" && node.operator === "-") {
    return isNumericNode(node.argument);
  }
  return false;
}
...
function validate(ast, filePath, source) {
  ...
  walk.full(ast, (node) => {
    ...
    /*
     * Catch string concatenation via "+".
     *
     * In C, char * + char * is pointer arithmetic, not string concatenation.
     * We must reject any "+" BinaryExpression where either operand is a
     * known string-typed AST node. Pure numeric + (both sides are numeric
     * literals or numeric-only sub-expressions) is allowed through.
     *
     * Use template literals (`Hello ${name}`) instead.
     */
    if (node.type === "BinaryExpression" && node.operator === "+") {
      if (isStringNode(node.left) || isStringNode(node.right)) {
        addError(
          node,
          `string concatenation with '+' is not supported because 'char * + char *' is invalid in C. ` +
            `Use a template literal instead: \`...\${expression}...\``,
        );
      } else if (!isNumericNode(node.left) || !isNumericNode(node.right)) {
        /*
         * At least one side is an identifier or other ambiguous expression.
         * We cannot safely determine its C type at compile time, so reject it
         * to prevent a cryptic gcc error.
         */
        addError(
          node,
          `'+' with non-literal operands is ambiguous: cerver cannot determine whether ` +
            `this is numeric addition or string concatenation. ` +
            `For string values use a template literal (\`...\${expression}...\`); ` +
            `for numeric values use only numeric literals or arithmetic sub-expressions.`,
        );
      }
      /* Both sides purely numeric — allowed, falls through to IR as arithmetic */
    }
  });
