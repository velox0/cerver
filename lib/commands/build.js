...
function resolveCompileOptions(config, opts, usesFetch) {
  const cli = opts || {};
  const compileConfig =
    config && typeof config.compile === "object" && config.compile !== null
      ? config.compile
      : {};

  const target = cli.target || compileConfig.target || null;
  const targetOs = cli.targetOs || compileConfig.targetOs || null;
  const targetArch = cli.targetArch || compileConfig.targetArch || null;
  const resolvedTarget =
    target || (targetArch && targetOs ? `${targetArch}-${targetOs}` : null);

  const marchNative =
    cli.marchNative !== undefined
      ? cli.marchNative
      : compileConfig.marchNative !== undefined
        ? compileConfig.marchNative
        : !resolvedTarget;

  return {
    static: cli.static,
    usesFetch,
    cc: cli.cc || compileConfig.cc,
    output: cli.output || compileConfig.output,
    target: resolvedTarget,
    sysroot: cli.sysroot || compileConfig.sysroot,
    cflags: cli.cflags !== undefined ? cli.cflags : compileConfig.cflags,
    ldflags: cli.ldflags !== undefined ? cli.ldflags : compileConfig.ldflags,
    lto: cli.lto !== undefined ? cli.lto : compileConfig.lto,
    marchNative,
    compileInfo: cli.compileInfo || compileConfig.compileInfo,
  };
}
...
async function build(opts) {
  ...
  /* CLI overrides */
  if (cli.embed !== undefined) config.embed = cli.embed;
  if (cli.minify === false) config.minify = false;
  ...
  if (routeFiles.length === 0) {
    console.warn(
      `  ⚠ no route files found in ${path.relative(projectDir, routesDir)}/`,
    );
  } else {
    for (const r of routeFiles) {
      console.log(
        `    ${r.urlPath} ← ${path.relative(projectDir, r.filePath)}`,
      );
    }
  }
  ...
  if (config.embed) {
    ...
    if (assets.length > 0) {
      ...
      for (let i = 0; i < Math.min(assets.length, 5); i++) {
        const a = assets[i];
        console.log(`    ${a.servePath} (${(a.size / 1024).toFixed(1)} KB)`);
      }
      if (assets.length > 5) {
        console.log(`    ... and ${assets.length - 5} more assets`);
      }
      assetsCode = await generateEmbeddedAssets(
        assets,
        config.minify,
        config.compression,
      );
  ...
  console.log(
    `    wrote ${(serverC.length / 1024).toFixed(1)} KB → dist/server.c`,
  );

  const routesCPath = path.join(distDir, "routes.c");
  fs.writeFileSync(routesCPath, routesC);
  console.log(
    `    wrote ${(routesC.length / 1024).toFixed(1)} KB → dist/routes.c`,
  );
  ...
  if (assetsCode) {
    ...
    console.log(
      `    wrote ${(assetsCode.length / 1024).toFixed(1)} KB → dist/assets.c`,
    );
  } else {
  ...
  const compileOptions = resolveCompileOptions(config, cli, usesFetch);
  if (compileOptions.output) {
    compileOptions.output = path.resolve(projectDir, compileOptions.output);
  }
  const binaryPath = compileC(distDir, runtimeDir, compileOptions);
