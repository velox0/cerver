...
const { copyAssets } = require("../assets/copy");
...
async function build(opts) {
  ...
  if (config.embed) {
    ...
    /* embed: false — copy public/ to dist/public/ so the binary finds them
     * at ./public relative to its working directory (dist/). */
    const destDir = path.join(distDir, "public");
    console.log("  → copying assets to dist/public/...");
    if (fs.existsSync(publicDir)) {
      const count = await copyAssets(publicDir, destDir, config.minify);
      console.log(`    ${count} file(s) copied → dist/public/`);
    } else {
